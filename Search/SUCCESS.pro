...
process SUCCESS(int trial_length,						// see DEFAULT.pro and ALL_VARS.pro for explanations of these globals
	...
	declare hide int 	nJuiceCount;
	declare hide int 	nJuiceOld;
	...
	if (State == run_search_sess)
		...
		if (isBonus)
		{
			nJuiceOld = nJuiceGive;
			nJuiceGive = bonusSize;
		}			
		nJuiceCount = 0;
		while (nJuiceCount < nJuiceGive)
		{
			spawnwait JUICE(juice_channel,reward_duration);			// YEAH BABY!  THAT'S WHAT IT'S ALL ABOUT!
			nJuiceCount = nJuiceCount+1;
			if (nJuiceCount < nJuiceGive)
			{
				wait 100;
				nexttick;
			}
