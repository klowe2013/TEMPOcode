...
declare DRW_ELL(float horizLn, float vertLn, float angle, float eccentricity, int color, int fill, float conversion_X, float conversion_Y);

process DRW_ELL(float horizLn, float vertLn, float angle, float eccentricity, int color, int fill, float conversion_X, float conversion_Y)
{
	...
	// find half sizes in each dimension (here, this is x radius and y radius for h and v respectively)
	half_sizeH = horizLn/2;
	half_sizeV = vertLn/2;
	
	// The below was necessary for drawing rectangles because they need left, right, 
	/*
