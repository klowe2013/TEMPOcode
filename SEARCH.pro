...
process SEARCH()
	...
	{
		 // check if current trial is within a US stimulation block
         // Blocks are defined as time intervals with US On or US Off
         // time intervals have the same duration (StimInterval)
		 if (time() > (LastStim + StimInterval))	// for blocked, on-task stimulation
		 {
			  LastStim = time();  // keep time when block state changed

			  if(StimCond == 1) // currently stimulation is on, thus switch it off
		  	  {
				  StimCond = 0;   // switch off US stimulation

				  Event_fifo[Set_event] = EndStim_;
				  Set_event = (Set_event + 1) % Event_fifo_N;
			  }
			  else // currently stimulation is off, thus switch it on
			  {
				  StimCond = 1;   // switch on US stimulation
