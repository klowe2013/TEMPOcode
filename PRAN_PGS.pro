//--------------------------------------------------------------------------------------------------
// process PRAN_PGS(int curr_target, 
				// float fixation_size, 
				// int fixation_color, 
				// int sig_color, 
				// float scr_width, 
				...
				// float pd_bottom, 
				// float pd_size);
// Figure out all stimuli that will be needed on the next pro/anti trial and
// place it all into video memory.
//
// Started by kaleb.a.lowe@vanderbilt.edu October 2010
//
// Based on LSCH_PGS
...
declare PRAN_PGS(int curr_target,
				int curr_wd,													// set SETC_TRL.pro
...
process PRAN_PGS(int curr_target, 
	...
	if (SetSize > 0)
		...
		if (isPro == 0)
			{
			spawnwait DRW_RECT(targ_angle, targ_ecc, color, targ_orient, fill, deg2pix_X, deg2pix_Y);          	// draw target
			}
		else if (isPro == 1)
			{
